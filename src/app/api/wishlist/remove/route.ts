...
export async function GET(request: Request) {
  ...
  const wishlistCookie = await globalFetcher({
    url: `${process.env.API_URL}/mywishlist/v2/remove-wishlist-item/?product_id=${product_id}`,
    method: "DELETE",
    apiType: "user",
  });
  ...
  if (d.status) {
    ...
    const indexOfProductId = cookieArray.indexOf(+product_id!);
    cookieArray.splice(indexOfProductId, 1);
