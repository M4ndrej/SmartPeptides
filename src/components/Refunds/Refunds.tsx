const RefundsAndReturns = () => {
  return (
    <>
      <div className="container-padding-inline m-[auto] mt-[48px] w-[100%] max-w-[1264px] sm:max-w-[100vw] md:max-w-[100vw]">
        <h1 className="font-D32px-M24px pb-[16px] text-center font-bold">
          Refunds and Returns
        </h1>
        <div className="mx-auto mb-[20px] h-[2px] w-[48px] bg-[#E7461E]"></div>

        <div className="font-16px-ALL container-margin-bottom-D96px-M64px">
          <div className="font-D24px-M18px mb-[20px] text-[#E7461E]">
            Do You Accept Returns?
          </div>

          <div className="mb-[20px]">
            valuepeptide.com does not accept returns for any products. Our items
            require strict handling and quality control, and because of their
            sensitive nature, we cannot receive or process returns under any
            circumstances.
          </div>

          <div className="font-D24px-M18px mb-[20px] text-[#E7461E]">
            Problems With Your Order
          </div>

          <div className="mb-[20px]">
            If something is wrong with your order, such as a missing, damaged,
            or incorrect item, please contact us right away at
            support(at)valuepeptide.com. Our support team will review the issue
            and help you find a quick solution.
          </div>

          <div className="font-D24px-M18px mb-[20px] text-[#E7461E]">
            Canceling an Order
          </div>

          <div className="mb-[20px]">
            If you need to cancel an order, you must email us before the package
            has been shipped. Once the order has been handed over to the
            carrier, cancellations can no longer be processed.
          </div>

          <div className="font-D24px-M18px mb-[20px] text-[#E7461E]">
            Lost Packages
          </div>

          <div className="mb-[20px]">
            In rare cases where a shipping carrier marks a package as lost or
            undeliverable, we will send a replacement at no extra cost. If the
            tracking information shows the package as successfully delivered,
            our responsibility is considered complete and a free replacement
            cannot be provided.
          </div>

          <div className="font-D24px-M18px mb-[20px] text-[#E7461E]">
            Missing Items
          </div>

          <div className="mb-[20px]">
            If an item is missing from your shipment, reach out to us
            immediately at support(at)valuepeptide.com. After confirming that
            the claim is valid, we will send the missing item promptly. For
            accuracy and fraud prevention, every order is photographed and
            documented before it leaves our facility.
          </div>

          <div className="font-D24px-M18px mb-[20px] text-[#E7461E]">
            Need Help?
          </div>

          <div className="mb-[20px]">
            If you have any other questions or concerns about your order, feel
            free to email us at support@valuepeptide.com. Our team is here to
            assist you as quickly as possible.
          </div>
        </div>
      </div>
    </>
  );
};

export default RefundsAndReturns;
